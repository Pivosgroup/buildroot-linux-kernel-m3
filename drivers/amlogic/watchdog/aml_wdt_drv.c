...
#include "aml_wdt_drv.h"

#define AML_WDT_PING_TIMEOUT        1   /* 1 seconds  */
#define AML_WDT_STARTUP_TIMEOUT     10  /* 10 seconds */
#define AML_WDT_USER_PET_TIMEOUT    30  /* 30 seconds */
...
static unsigned int user_pet_reset_enable = 0; /* reset when software watchdog is triggered */
 ...
 */
static inline void aml_wdt_set_enable(bool enalbe)
{
    ...
    WRITE_CBUS_REG(WATCHDOG_TC, ((enalbe ? 1 : 0) << WATCHDOG_ENABLE_BIT) | timeout);
}
...
static ssize_t aml_wdt_reset_enable_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%s\n", (aml_wdt_get_enable() ? "enable" : "disable"));
}
...
static int aml_wdt_probe(struct platform_device *pdev)
{
    ...
    aml_wdt_settimeout(AML_WDT_STARTUP_TIMEOUT);
    aml_wdt_set_enable(true);
