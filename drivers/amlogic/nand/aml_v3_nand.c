...
static void aml_platform_hw_init(struct aml_nand_chip *aml_chip)
{
	...
	if ((aml_chip->chip_num > 1) && !nand_erarly_suspend_flag) {
		...
		CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << 5));
		CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, (1 << 5));
		SET_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << 16));
		if (!(READ_CBUS_REG(PREG_PAD_GPIO3_I) & (1 << 16))) {
			SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, (1 << 5));
			if ((READ_CBUS_REG(PREG_PAD_GPIO3_I) & (1 << 16))) {
	...
	NFC_SEND_CMD(1<<31);
	dev_info(aml_chip->device, "time_mode=%d, bus_cycle=%d, tREA=%d, tRHOH=%d, bus_tim=%x system=%d\n",
...
{
	int ret = 0;
	unsigned count=0,pgsz=0;
	//struct mtd_info *mtd = &aml_chip->mtd;
	struct nand_chip *chip = &aml_chip->chip;
	dma_addr_t data_dma_addr=0;
	dma_addr_t user_data_dma_addr=0;
...
//	data_dma_addr=dma_map_single(aml_chip->device,(void *)buf,len,DMA_TO_DEVICE);	
//	user_data_dma_addr=dma_map_single(aml_chip->device,(void *)aml_chip->user_info_buf,count*PER_INFO_BYTE,DMA_TO_DEVICE);
	...
	NFC_SEND_CMD_ADH(aml_chip->aml_nand_dma_buf_dma_addr);
	NFC_SEND_CMD_AIL(aml_chip->aml_nand_info_dma_addr);
	NFC_SEND_CMD_AIH(aml_chip->aml_nand_info_dma_addr);	
...
//	dma_unmap_single(aml_chip->device,data_dma_addr,count*PER_INFO_BYTE,DMA_FROM_DEVICE);
...
//	dma_unmap_single(aml_chip->device,data_dma_addr,(mtd->writesize/chip->ecc.size)*PER_INFO_BYTE,DMA_FROM_DEVICE);
//	
...
static void aml_platform_nand_suspend(struct early_suspend *nand_early_suspend)
{
	...
	aml_nand_debug("aml_m1_nand_early suspend entered\n");
	return;
}
...
static void aml_platform_nand_resume(struct early_suspend *nand_early_suspend)
{
	...
	aml_nand_debug("aml_m1_nand_early resume entered\n");
	return;
}
#endif
...
static int aml_nand_suspend(struct mtd_info *mtd)
{
	...
	aml_nand_debug("aml_nand suspend entered\n");
	return 0;
}
...
static void aml_nand_resume(struct mtd_info *mtd)
{
	...
	aml_nand_debug("aml_m1_nand resume entered\n");
	return;
}
