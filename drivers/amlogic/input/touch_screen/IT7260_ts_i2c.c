...
static struct input_dev* IT7260_register_input(int xres, int yres)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, xres, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, yres, 0, 0);
	...
	input_set_abs_params(input_dev, ABS_X, 0, xres, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, yres, 0, 0);
...
static int IT7260_ts_probe(struct i2c_client *client,
	...
	struct input_dev *input_dev = IT7260_register_input(pdata->xres, pdata->yres);
	if (!input_dev) {
		return -ENODEV;
	}
	...
	if (client->irq) {
		...
		gpio_enable_irq(pdata->irq_gpio, client->irq, pdata->irq_flag);
		ret = request_irq(client->irq, IT7260_ts_irq_handler, pdata->irq_flag,
