...
#endif
#define GPIO_IRQ ((GPIOA_bank_bit0_27(16)<<16) | GPIOA_bit_bit0_27(16))
//#define SWAP_XY
//#define XPOL
//#define YPOL
...
#ifdef SWAP_XY
		swap(TPInfo->pt[i].x, TPInfo->pt[i].y);
#endif
#ifdef XPOL
		TPInfo->pt[i].x = SIS_MAX_X - TPInfo->pt[i].x;
#endif
#ifdef YPOL
		TPInfo->pt[i].y = SIS_MAX_Y - TPInfo->pt[i].y;
#endif
	}
#if 0
...
label_send_report:
    /* Report co-ordinates to the multi-touch stack */
		...
		for(i = 0; ((i < TPInfo->fingers) && (i < MAX_FINGERS)); i++)
		{
			...
			//input_report_key(ts->input_dev, BTN_TOUCH, TPInfo->pt[i].bPressure);
...
//		input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, TPInfo->pt[i].id);
		...
		//input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, TPInfo->pt[i].id);	//Android 2.3
		input_mt_sync(ts->input_dev);
		//printk("point[%d]: x=%d, y=%d, z=%d\n", i/*TPInfo->pt[i].id*/, TPInfo->pt[i].x, TPInfo->pt[i].y, TPInfo->pt[i].bPressure);
		i++;
	}
...
static int initial_irq(int gpio, int irq)
{
	int ret = 0;
#ifdef _I2C_INT_ENABLE
	/* initialize gpio and interrupt pins */
	/* TODO */
	ret = gpio_request(gpio, "GPIO_133");	// ex. GPIO_133 for interrupt mode
	if (ret < 0)
	{
    ...
    gpio_enable_edge_int(gpio_to_idx(gpio), 1, irq-INT_GPIO_0);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->input_dev->name = SIS_I2C_NAME;//"SiS 9200 family I2C touchscreen";
...
//	}
	...
	//sis_reset();
	ret = initial_irq(GPIO_IRQ, client->irq);
	...
	else
	{
		...
		if (ret == 0) {
		    printk("request irq successed(%d)\n", client->irq);
		   ts->use_irq = 1;
		}
		else {
