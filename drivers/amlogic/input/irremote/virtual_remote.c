...
static int
virtual_remote_open(struct inode *inode, struct file *file)
{
                 ...
                 unsigned int cmd, unsigned long args)
{
       ...
       switch(cmd){
        ...
        case 1 :
            copy_from_user(&scan_code, (unsigned int*)args, sizeof(unsigned int));
...
static int 
virtual_remote_release(struct inode *inode, struct file *file)
{
...
static int __init virtual_remote_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Unable to register Virtual Remote input device\n");
    	       kfree(apollo_kp);
	       input_free_device(input_dev);
