...
static inline int get_pulse_width(unsigned long data)
{
    ...
    int num;
    int ret;
    unsigned int range[2] = {kp_data->time_window[2]>>1, kp_data->time_window[3]>>1};
    ...
    if(pulse_width > num*range[0] && pulse_width < num*range[1])
        ret = num;
    else if(pulse_width > (num+1)*range[0] && pulse_width < (num+1)*range[1])
        ret = num+1;
    else
        ret = 0;        
    
	state = kp_data->step==RC6_STATUS_WAIT?"wait":\
				kp_data->step==RC6_STATUS_LEADER?"leader":\
				kp_data->step==RC6_STATUS_START?"start":\
				kp_data->step==RC6_STATUS_MODE?"mode":\
                kp_data->step==RC6_STATUS_TRAILER?"trailer":\
                kp_data->step==RC6_STATUS_CONTROL?"control":\
                kp_data->step==RC6_STATUS_INFO?"information":\
                kp_data->step==RC6_STATUS_FREE?"free":NULL;
    dbg_printk("%02d:pulse_wdith:%d==>%s\r\n", kp_data->bit_count - kp_data->bit_num,ret,state);
...
static inline void kbd_rc6_wait(unsigned long data, win_time_t* window)
{
    ...
    kp_data->step = RC6_STATUS_LEADER;
...
static inline void kbd_rc6_trailer(unsigned long data, win_time_t* window)
{
    ...
    else if(sub_status == 1 && window->time >= 2 && trailer_level != window->level){
...
static inline void kbd_rc6_info(unsigned long data, win_time_t* window)
{
    ...
    if(kp_data->bit_num == 0){//send the key
...
void kp_rc6_reprot_key(unsigned long data)
{
    ...
    if(((current_jiffies - kp_data->last_jiffies) > 20) && (kp_data->step  <=  RC6_STATUS_FREE)){
        kp_data->step = RC6_STATUS_WAIT;
    }
...
irqreturn_t remote_rc6_bridge_isr(int irq, void *dev_id)
{
	...
	{
	    kp_data->step = RC6_STATUS_SYNC;
        kp_send_key(kp_data->input, kp_data->cur_keycode, 1);
		kp_data->send_data=0;
	}
