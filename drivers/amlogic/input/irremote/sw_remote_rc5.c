...
static inline int get_pulse_width(unsigned long data)
{
	...
	int num;
	int ret;
	unsigned int range[2] = {kp_data->time_window[2], kp_data->time_window[3]};
	...
	state = kp_data->step==REMOTE_STATUS_WAIT?"wait":\
		kp_data->step==RC5_STATUS_START?"start":\
		kp_data->step==RC5_STATUS_INDICATION?"indication":\
		kp_data->step==RC5_STATUS_CONTROL?"control":\
		kp_data->step==RC5_STATUS_SYSTEM?"system":\
		kp_data->step==RC5_STATUS_COMMAND?"command":\
		kp_data->step==RC5_STATUS_SYNC?"sync":NULL;
	dbg_printk("-----------------------pulse_wdith:%d  ret:%d   state:%s \n", pulse_width, ret, state);
	//printk("-----------------------pulse_wdith:%d  ret:%d   state:%s \n", pulse_width, ret, state);
	return ret;
}
...
static inline void kbd_rc5_control(unsigned long data, win_time_t* window)
{
	if(sub_status == 0){
		...
		if((bit_0_1 == 0 && rc5_bit0_high(window) == 0) || (bit_0_1 == 1 && rc5_bit1_low(window) == 0))
...
static inline void kbd_rc5_command(unsigned long data, win_time_t* window)
{
	...
	else if(kp_data->bit_num == 1){//the last bit is 0, so the high level can not be captured
...
void kp_rc5_reprot_key(unsigned long data)
{
	...
	if(((current_jiffies - kp_data->last_jiffies) > 20) && (kp_data->step  <=  RC5_STATUS_SYNC)){
