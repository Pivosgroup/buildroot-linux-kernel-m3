...
bool is_associated(struct net_device *dev, void *bss_buf)
{
	char bssid[6], zbuf[6];
	int res;
	...
	if ((res == 0) && (memcmp(bssid, zbuf, ETHER_ADDR_LEN) != 0)) {
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		WL_ASSOC(("%s rssi;%d, lnk_down_was_sent:%d\n",
			__FUNCTION__, rssi, lnk_down_was_sent));

		if (!error && !lnk_down_was_sent &&
			!is_associated(dev, NULL)) {

			union iwreq_data w_evnt;
			int cmd = SIOCGIWAP;

			lnk_down_was_sent = TRUE; 
			WL_ERROR(("%s: Link down evnt was lost, resend it\n", __FUNCTION__));
...
static void wl_iw_arp_cleanup(struct net_device *dev)
{
	...
	if ((ret  = dev_wlc_ioctl(dev, WLC_SET_VAR, iovbuf, iov_len)) < 0)
		WL_ERROR(("%s failed code %d\n", __FUNCTION__, ret));
