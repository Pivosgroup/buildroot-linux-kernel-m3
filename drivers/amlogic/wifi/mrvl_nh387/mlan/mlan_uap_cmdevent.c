    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME) {
            ...
            if ((mgmt_tlv->frame_control.type == 0) &&
                ...
                if (mgmt_tlv->frame_control.sub_type == SUBTYPE_ASSOC_REQUEST)
                    assoc_ie_len = sizeof(IEEEtypes_AssocRqst_t);
                else if (mgmt_tlv->frame_control.sub_type ==
                         SUBTYPE_REASSOC_REQUEST)
...
/**
 *  @brief This function prepares command of rx data pause
 *  
 *  @param pmpriv		A pointer to mlan_private structure
 *  @param cmd	   		A pointer to HostCmd_DS_COMMAND structure
 *  @param cmd_action   the action: GET or SET
 ...
 */
mlan_status
wlan_uap_cmd_rx_pause(pmlan_private pmpriv,
                      IN HostCmd_DS_COMMAND * cmd,
                      IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    HostCmd_DS_RX_DATA_PAUSE *prx_pause =
        (HostCmd_DS_RX_DATA_PAUSE *) & cmd->params.data_pause;
    ENTER();

    cmd->command = wlan_cpu_to_le16(HOST_CMD_RX_DATA_PAUSE);
    cmd->size = wlan_cpu_to_le16(S_DS_GEN + sizeof(HostCmd_DS_RX_DATA_PAUSE));

    prx_pause->rx_pause = wlan_cpu_to_le16(pmpriv->adapter->rx_pause_flag);
    prx_pause->action = wlan_cpu_to_le16(cmd_action);
...
#ifdef DEBUG_LEVEL1
/** 
 *  @brief This function handles the command response of rx data pause
 *  
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param resp         A pointer to HostCmd_DS_COMMAND
 *
 ...
 */
mlan_status
wlan_uap_ret_rx_pause(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND * resp)
{

    HostCmd_DS_RX_DATA_PAUSE *prx_pause =
        (HostCmd_DS_RX_DATA_PAUSE *) & resp->params.data_pause;
    t_u16 rx_pause = 0;

    ENTER();

    rx_pause = wlan_le16_to_cpu(prx_pause->rx_pause);
    PRINTM(MCMND, "CMDRESP: rx_pause=%d, rx_pause_flag=%d pending_pkts=%d\n",
           rx_pause, pmpriv->adapter->rx_pause_flag,
           pmpriv->adapter->pending_bridge_pkts);
                     ...
                     IN t_void * pdata_buf, IN t_void * pcmd_buf)
{
    ...
    case HOST_CMD_APCMD_SYS_CONFIGURE:
        ...
        break;
    case HOST_CMD_RX_DATA_PAUSE:
        ret = wlan_uap_cmd_rx_pause(pmpriv, cmd_ptr, cmd_action, pdata_buf);
        break;
                         ...
                         IN t_void * pcmd_buf, IN t_void * pioctl)
{
    ...
    case HOST_CMD_RX_DATA_PAUSE:
#ifdef DEBUG_LEVEL1
        ret = wlan_uap_ret_rx_pause(pmpriv, resp);
#endif /* DEBUG_LEVEL1 */
        break;
    default:
...
mlan_uap_init_cmd(IN t_void * priv, IN t_u8 first_sta)
{
     ...
     */
    if (pmpriv->adapter->int_mode == INT_MODE_GPIO) {
