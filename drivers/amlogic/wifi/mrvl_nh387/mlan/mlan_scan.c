    ...
    for (region_idx = 0;
        ...
        for (next_chan = 0;
            ...
            if (scan_type == MLAN_SCAN_TYPE_ACTIVE
                && wlan_11d_get_state(pmpriv) == ENABLE_11D) {
            ...
            case BAND_A:
                ...
                if (wlan_11d_get_state(pmpriv) == DISABLE_11D) {
                       ...
                       IN ChanScanParamSet_t * pscan_chan_list)
{
    ...
    while (ptmp_chan_list->chan_number) {
        ...
        while (tlv_idx < max_chan_per_scan && ptmp_chan_list->chan_number &&
            ...
            memcpy(pmpriv->adapter, pchan_tlv_out->chan_scan_param + tlv_idx,
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf) {
        ...
        if (new_bcn_size == old_bcn_size) {
            ...
            if (old_bcn_size < bcn_space && new_bcn_size <= old_bcn_size) {
                 ...
                 */
                memmove(pmadapter, pbcn_store + old_bcn_size,
             ...
             *  about to copy in.
             */
            memmove(pmadapter, pbcn_store + new_bcn_size,
                    pbcn_store + bcn_space,
                    pmadapter->pbcn_buf_end - (pbcn_store + bcn_space));
        ...
        if (!wlan_ssid_cmp(pmadapter, &pmadapter->pscan_table[i].ssid, ssid) &&
            ...
            (wlan_find_cfp_by_band_and_channel
             (pmadapter, (t_u8) pmadapter->pscan_table[i].bss_band,
              (t_u16) pmadapter->pscan_table[i].channel))) {
            switch (mode) {
            case MLAN_BSS_MODE_INFRA:
            case MLAN_BSS_MODE_IBSS:
    ...
    for (i = 0; net < 0 && i < pmadapter->num_in_scan_table; i++) {
        if (!memcmp
            (pmadapter, pmadapter->pscan_table[i].mac_address, bssid,
             MLAN_MAC_ADDR_LENGTH) &&
            wlan_find_cfp_by_band_and_channel(pmadapter,
                                              (t_u8) pmadapter->pscan_table[i].
                                              bss_band,
                                              (t_u16) pmadapter->pscan_table[i].
                                              channel)) {
            switch (mode) {
            case MLAN_BSS_MODE_INFRA:
            case MLAN_BSS_MODE_IBSS:
                         ...
                         IN t_u16 vsie_mask, OUT t_u8 ** ppbuffer)
{
    ...
    for (id = 0; id < MLAN_MAX_VSIE_NUM; id++) {
        if (pmpriv->vs_ie[id].mask & vsie_mask) {
            ...
            memcpy(pmpriv->adapter, pvs_param_set->ie, pmpriv->vs_ie[id].ie,
                   pvs_param_set->header.len);
