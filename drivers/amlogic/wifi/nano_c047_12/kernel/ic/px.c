...
int
fw_read_file(struct nrx_px_softc *sc, 
		 const char *filename)
{
   int status;
   const struct firmware *fw;
   struct net_device *dev = nrx_px_priv(sc);
   struct nrx_softc *nrx_sc = netdev_priv(dev);

   status = request_firmware(&fw, filename, nrx_sc->pdev);
   if(status == 0 && fw != NULL) {
   		printk("Request fw OK!\n");
   }
   else {
   		printk("Request fw fail!\n");
	   return status;
   }

   if((status = nrx_px_allocate(sc, fw->size)) < 0) {
      printk("nrx_px_allocate fail: sts = %d, sz = %d\n", status, fw->size);
      release_firmware(fw);
      return status;
   }

	memcpy(sc->px_buf, fw->data, fw->size);
	NRX_PX_SETSIZE(sc, fw->size);
	release_firmware(fw);
