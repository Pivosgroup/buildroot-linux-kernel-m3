...
mmc_set_clock(struct mmc_host *host, unsigned clk)
{
   ...
   return prev_clk;
	...
	prev_clk = clk_get_rate(clk_get_sys("clk81", NULL))/2/(config_reg->cmd_clk_divide + 1);
	printk("Prev clk = %dM\n", prev_clk/1000000);
	if(clk==0)
	{
		printk("Try to disable clock!\n");
		goto _f_exit;
	}
	
	config_reg->cmd_clk_divide = (clk_get_rate(clk_get_sys("clk81", NULL))/2/clk) -1;
	WRITE_CBUS_REG(SDIO_CONFIG, sdio_config);

	printk("cmd_clk_divide %d, Current CLK %ldM\n", config_reg->cmd_clk_divide, 
...
nrx_init(struct sdio_func *func) 
{
   ...
   printk("Card using SDIO address %u\n", rca);
   func->card->rca = rca;
   ...
   if (err) return err;
#else
   extern int sdio_read_rca(struct memory_card *card, unsigned* rca);
   extern int sdio_select_card(struct memory_card *card);

   unsigned rca = 0;
   err = sdio_read_rca(func->card, &rca);
   printk("sdio_read_rca() = %d\n", err);
   if (err) return err;

   printk("Card using SDIO address %u\n", rca);
   //func->card->rca = rca;

   err = sdio_select_card(func->card);
   printk("sdio_select_card() = %d\n", err);
   if (err) return err;
#endif
...
#else
	extern int sdio_rw_direct(struct memory_card  *card, unsigned int arg);
	err = sdio_rw_direct(func->card, 0x88000E02);
	printk("mmc_io_rw_direct(enable 4-bit SDIO) = %d\n", err);
#endif
#endif
...
#else
   err = sdio_rw_direct(func->card, 0x88000803);
   printk("mmc_io_rw_direct(enable IRQ) = %d\n", err);
#endif
#endif
...
static int sdio_nrx_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
   ...
   err = nrx_init(func);
   if (err) {
      printk("nano_ksdio: Failed to init target, err %d\n", err);
      sdio_disable_func(func);
      sdio_release_host(func);
      return err;
   }
   ...
   if (err) {
      printk("nano_ksdio: nrx_soft_shutdown failed, err %d\n", err);
      sdio_disable_func(func);
      sdio_release_host(func);
      return err;
   }
