...
static int dib7000p_debug=1;
module_param(dib7000p_debug, int, 0644);
MODULE_PARM_DESC(dib7000p_debug, "turn on debugging (default: 0)");

static int buggy_sfn_workaround=0;
module_param(buggy_sfn_workaround, int, 0644);
MODULE_PARM_DESC(buggy_sfn_workaround, "Enable work-around for buggy SFNs (default: 0)");

#define dprintk(args...) do { if (dib7000p_debug) { printk("DiB7000P: "); printk(args); printk("\n"); } } while (0)
...
static u16 dib7000p_read_word(struct dib7000p_state *state, u16 reg)
{
	...
	if (i2c_transfer(state->i2c_adap, state->msg, 2) != 2)
		printk("i2c read error on %d", reg);
...
#define printf printk
#if REG_PRINT
   {
   	int i;
       printf("-R- ");
       printf("0x%02x] ", state->msg[0].addr);
       for(i=0;i<2;i++)
       {
           printf("[w 0x%02x]", state->i2c_write_buffer[i]);
       }
       for(i=0;i<2;i++)
       {
           printf(" [r 0x%02x]", state->i2c_read_buffer[i]);
       }

       if(2 == 1)
       {
           printf(" [%d: ", state->i2c_write_buffer[0]);
       printf(" %d]", (state->i2c_read_buffer[0]<<8)|state->i2c_read_buffer[1]);
       }
       else
       {
           printf(" [%d: ", (state->i2c_write_buffer[0]<<8)|state->i2c_write_buffer[1]);
           printf(" %d]", (state->i2c_read_buffer[0]<<8)|state->i2c_read_buffer[1]);
       }
           printf("\n");
           //printf("\n");
...
#define printf printk
#if REG_PRINT
   {
       ...
       printf("-W- ");
       printf("0x%02x] ", state->msg[0].addr);
       printf("[%ld]", state->msg[0].len);
       for(i=0;i<4;i++)
       {
           printf(" 0x%02x ", state->i2c_write_buffer[i]);
       }

       if(4==3)
       {
           printf(" [%d: %d]", state->i2c_write_buffer[0], (state->i2c_write_buffer[1]<<8)|state->i2c_write_buffer[2]);
       }
       else
       {
           printf(" [%d: %d]", (state->i2c_write_buffer[0]<<8)|state->i2c_write_buffer[1], (state->i2c_write_buffer[2]<<8)|state->i2c_write_buffer[3]);
       }
       printf("\n");
       //printf("\n");
   }
...
static int dib7000p_set_output_mode(struct dib7000p_state *state, int mode)
{
	...
	printk("setting output mode for demod %p to %d", &state->demod, mode);
	...
	case OUTMODE_HIGH_Z:	// disable
		...
		break;
	default:
		printk("Unhandled output_mode passed to be set for demod %p", &state->demod);
		break;
	}
...
static int dib7000p_set_diversity_in(struct dvb_frontend *demod, int onoff)
{
	...
	if (state->div_force_off) {
		printk("diversity combination deactivated - forced by COFDM parameters");
		onoff = 0;
...
int dib7000p_update_pll(struct dvb_frontend *fe, struct dibx000_bandwidth_config *bw)
{
	...
	if ((bw != NULL) && (bw->pll_prediv != prediv || bw->pll_ratio != loopdiv)) {
		...
		while (((dib7000p_read_word(state, 1856) >> 15) & 0x1) != 1)
			printk("Waiting for PLL to lock");
...
static int dib7000p_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *fep)
{
	...
	printk("dib7000p_set_frontend : f[%d] i[%d] b[%d]\n", fep->frequency, fep->inversion, fep->u.ofdm.bandwidth);
	if (state->version == SOC7090)
